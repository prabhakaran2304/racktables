...
function localfunc_HPWarranty()
{
	...
	foreach (getAttrValues ($object['id'], TRUE) as $record)
		if (strlen ($record['value']) && $record['name'] == "Productnumber")
			$hppn = $record['value'];
	if ($object['barcode'])
	renderIframeTabForEntity("HP warranty", "https://support.hp.com/us-en/checkwarranty".$object['barcode']."&country=DE&lang=de&cc=de&pn=".$hppn."&find=Display+Warranty+Information+%C2%BB&");
}
...
function localfunc_DellWarranty()
{
	assertUIntArg ('object_id', __FUNCTION__);
	$object = spotEntity ('object', $_REQUEST['object_id']);
	renderIframeTabForEntity("Dell warranty", "http://www.dell.com/support/home/in/en/bsdt1/Products/?app=warranty".$object['barcode']);
}
...
function renderIframeTabForEntity ($title, $link)
{
	...
	echo "<iframe border=0 src=\"$link\" width=%150 height=%99 halign=center style='width: expression(document.documentElement.clientWidth); height: expression(document.documentElement.clientHeight-510)'>";
	echo "</div>";
